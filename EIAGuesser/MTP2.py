...
def get_oof_predictions(X, y, models, cv_splitter):
    ...
    for i, model in enumerate(models):
        # We need a new splitter for generating OOF preds on the training set
...
for region, tgt_col in target_map.items():
    ...
    eval_tscv = TimeSeriesSplit(n_splits=5, test_size=12)
    fold_scores = []
    ...
    final_oof_preds = get_oof_predictions(
        X_all_np, y_all_np, [clone(m) for m in base_models], TimeSeriesSplit(n_splits=5)
    )
